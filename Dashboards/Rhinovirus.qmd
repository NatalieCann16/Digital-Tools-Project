...
## Overall University Report for Rhinovirus (Common Cold)
...
# Create mock rhinovirus data for ~40,000 students
set.seed(789)
rhino_data <- data.frame(
  Date = seq.Date(from = as.Date("2025-01-01"), by = "month", length.out = 12)
) %>%
  mutate(
    Month = as.integer(format(Date, "%m")),
    # Rhinovirus peaks in fall/spring, moderate in winter, low in summer
    Cases = case_when(
      Month %in% c(9, 10, 11, 2, 3, 4) ~ sample(1200:2000, 1),  # peaks fall & spring
      Month %in% c(12, 1) ~ sample(800:1200, 1),                # winter moderate
      Month %in% c(5, 6, 7, 8) ~ sample(300:700, 1)             # summer dip
    )
  ) %>%
  mutate(
...
rhino_plot <- ggplot(rhino_data, aes(x = Date)) +
  ...
  labs(
    title = "Rhinovirus (Common Cold) Cases \n\ (Monthly vs. Cumulative)",
...
active_cases_rhino <- round(last(rhino_data$Cases) * 0.25)

# Assume "past 72 hours" ~ 10% of the most recent month's cases
cases_72h_rhino <- round(last(rhino_data$Cases) * 0.10)
...
htmltools::tags$div(
  ...
  htmltools::tags$p(paste("Active Cases:", format(active_cases_rhino, big.mark=","))),
  htmltools::tags$p(paste("Cases in Past 72 Hours:", format(cases_72h_rhino, big.mark=",")))
)
```
:::
...
# Build the table
rhino_table <- rhino_data %>%
  mutate(Month_Name = format(Date, "%B")) %>%
  select(Month_Name, Cases, Cumulative)

# Create gt table
gt_rhino <- rhino_table %>%
  gt() %>%
  tab_header(title = "Monthly and Cumulative Rhinovirus Cases (2025)") %>%
  cols_label(
    Month_Name = "Month",
    Cases = "Monthly Cases",
  ...
  # Keep same alternating rows as flu
  tab_style(
    style = cell_fill(color = "#E9F5F0"),
    locations = cells_body(rows = seq(1, nrow(rhino_table), 2))
  ) %>%
  tab_style(
    style = cell_fill(color = "#f0f8ff"),
    locations = cells_body(rows = seq(2, nrow(rhino_table), 2))
  )
...
htmltools::tags$div(
  ...
  htmltools::tags$a(
    href = "https://www.cdc.gov/rhinoviruses/index.html",
    target = "_blank",
    style = "color:black; font-weight:bold; text-decoration:none;",
    "Visit CDC Rhinovirus Page"
  )
)
```
:::
...
![Rhinovirus Infographic](Rhinovirusinfographic.png)